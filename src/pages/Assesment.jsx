...
export default function Assesment(props) {
    ...
    const fetchData = useCallback(async () => {
        ...
        if (id.length) {
            if (id.length > 4) {
                newProjectID = id.substring(0, 3) + "60-" + id.substring(id.length - 2, id.length)
            } else {
                newProjectID = id.substring(0, 2) + "60-" + id.substring(id.length - 2, id.length)
            }
        }
        ...
        let filterTeacher = res.data.responsible_assignment.find((r) => r.teacher_id === user.id)//2
        const newAssessment = res.data.assessment.filter((a) => a.responsible_assignment_id === filterTeacher.id)
    ...
    function handleAssesment(event, index, data) {// 0, 1
                            ...
                            <Row>
                                ...
                                <Col sm={2}>
                                    {isAssesment.submission.status === "Submitted" ? (
                                        <p><FiberManualRecordIcon className="successStatus" /><medium className="d-inline">On time</medium></p>
                                    ) : (
                                            <p><FiberManualRecordIcon color="secondary" /><medium className="d-inline">Late</medium></p>
                                        )}
                                </Col>
                            </Row>
